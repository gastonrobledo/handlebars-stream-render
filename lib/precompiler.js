...
module.exports.loadTemplates = function(opts, callback) {
  loadStrings(opts, (err, strings) => {
    if (err) {
      callback(err)
    } else {
      loadFiles(opts, (err, files) => {
        if (err) {
          callback(err)
        } else {
...
function loadStrings(opts, callback) {
  const strings = arrayCast(opts.string),
...
function loadFiles(opts, callback) {
  ...
  let extension = (opts.extension || 'handlebars').replace(/[\\^$*+?.():=!|{}\-[\]]/g, arg => `\\${arg}`)
  extension = new RegExp(`\\.${extension}$`)

  const ret = [],
        queue = (opts.files || []).map(template => ({ template, root: opts.root }))
  Async.whilst(() => queue.length, (callback) => {
    const { template: path, root } = queue.shift()
...
module.exports.cli = function(opts) {
  ...
  opts.templates.forEach((template) => {
    ...
    if (opts.map) {
      const consumer = new SourceMapConsumer(precompiled.map)
